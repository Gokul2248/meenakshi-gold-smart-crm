...
const FOLDER_NAME = 'Meenakshi Jewels Visitor Cards';
...
function setup() {
  ...
  if (!ss) { ss = SpreadsheetApp.create('Meenakshi Jewels Smart Visitor CRM'); props.setProperty(PROP_SHEET_ID, ss.getId()); }
...
function doGet(e) {
  ...
  const leads = values.slice(1).map(row => {
    ...
    return {timestamp:o.Timestamp,recordId:o['Record ID'],mobile:o.Mobile,shopName:o['Shop Name'],place:o.Place||'',customerType:o['Customer Type'],interests,notes:o.Notes,priority:o.Priority,followup:o['Follow-up'],source:o.Source,cardFileUrl:o['Card File URL']};
  });
  return json({ leads });
}
function doPost(e) {
  try {
    ...
    const now = new Date();
    const recordId = 'MJ-' + Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyyMMdd-HHmmss') + '-' + Math.floor(Math.random()*1000);
...
function deleteLead(recordId) {
  ...
  const headers = values[0], recordIndex = headers.indexOf('Record ID'), cardIndex = headers.indexOf('Card File URL');
  ...
  for (let i=values.length-1;i>=1;i--) {
    if (String(values[i][recordIndex])===String(recordId)) {
      ...
      if(cardUrl){const fileId=extractDriveFileId(cardUrl);if(fileId){try{DriveApp.getFileById(fileId).setTrashed(true)}catch(e){}}}
      sh.deleteRow(i+1); return json({ok:true,deletedRecordId:recordId});
    }
  }
  return json({error:'Lead not found.'});
}
function extractDriveFileId(url){const s=String(url||'');const m=s.match(/\/file\/d\/([a-zA-Z0-9_-]+)/)||s.match(/[?&]id=([a-zA-Z0-9_-]+)/);return m?m[1]:'';}
function ensurePlaceColumn(sh){const headers=sh.getRange(1,1,1,Math.max(sh.getLastColumn(),1)).getValues()[0];if(headers.indexOf('Place')>=0)return;const shopIndex=headers.indexOf('Shop Name');if(shopIndex>=0){sh.insertColumnAfter(shopIndex+1);sh.getRange(1,shopIndex+2).setValue('Place')}else sh.getRange(1,headers.length+1).setValue('Place');}
function saveCardImage(folder,dataUrl,recordId,shopName){const match=String(dataUrl).match(/^data:([^;]+);base64,(.+)$/);if(!match)throw new Error('Invalid card image data.');const mime=match[1],bytes=Utilities.base64Decode(match[2]);const ext=mime.indexOf('png')>=0?'png':mime.indexOf('webp')>=0?'webp':'jpg';const safeShop=String(shopName).replace(/[^a-z0-9_-]+/gi,'_').slice(0,50);return folder.createFile(Utilities.newBlob(bytes,mime,recordId+'_'+safeShop+'.'+ext));}
